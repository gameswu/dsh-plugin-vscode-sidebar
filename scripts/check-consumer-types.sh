...
# imports `dsh-plugin-vscode-sidebar/client/service` WITHOUT @types/node and with
...
ln -s "$(pwd)" "$WORK/node_modules/dsh-plugin-vscode-sidebar"

cat > "$WORK/check.ts" <<'EOF'
import { SIDEBAR_FEATURES, SIDEBAR_SERVICE_VERSION } from 'dsh-plugin-vscode-sidebar/client/service'
import type {} from 'dsh-plugin-vscode-sidebar/client/service'
import type {
  VscodeSidebarService, FileViewerDescriptor, OpenTabSeed, SidebarSettingsRenderProps,
  TabComponentProps, TabDescriptor,
} from 'dsh-plugin-vscode-sidebar/client/service'
import type { SessionScope, SidebarSnapshot, SidebarState, SidebarStore, SidebarTab } from 'dsh-plugin-vscode-sidebar/client/service'
import type { SidebarPrefs } from 'dsh-plugin-vscode-sidebar/client/service'

declare const ctx: { vscodeSidebar: VscodeSidebarService }
...
ctx.vscodeSidebar.registerTab(tab)
...
ctx.vscodeSidebar.registerFileViewer(viewer)
const seed: OpenTabSeed = { type: 'x:tab', title: 'X', meta: { a: 1 } }
ctx.vscodeSidebar.openTab(seed, { sessionId: 's1' })
ctx.vscodeSidebar.openFile({ sessionId: 's1' }, '/p/a.csv')
ctx.vscodeSidebar.updateTab('t', { meta: { b: 2 } })
const snap: SidebarSnapshot = ctx.vscodeSidebar.getSnapshot()
const prefs: SidebarPrefs = snap.prefs
const store: SidebarStore = null as unknown as SidebarStore
void prefs; void store; void ctx.vscodeSidebar.version; void ctx.vscodeSidebar.features
...
if grep -E "dsh-plugin-vscode-sidebar|lib/types|check\.ts" "$WORK/strict.log" | grep -v "node_modules/cordis\|node_modules/cosmokit"; then
  echo "[check-consumer-types] FAIL: errors in the dsh-plugin-vscode-sidebar declaration surface:" >&2
  grep -E "dsh-plugin-vscode-sidebar|lib/types|check\.ts" "$WORK/strict.log" >&2
  exit 1
fi
if [ "$STATUS" -ne 0 ]; then
  echo "[check-consumer-types] pass 2 note: strict mode only reports upstream declaration noise:"
  grep -v "dsh-plugin-vscode-sidebar\|lib/types\|check\.ts" "$WORK/strict.log" | head -5
fi
