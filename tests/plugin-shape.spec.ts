 ...
 * packages/ui/jsonrpc).
 */
describe('dsh-plugin-vscode-sidebar plugin export shape', () => {
  it('has the namespace-plugin export shape (no stray default) so the Loader keeps name/inject/Config/apply', () => {
    ...
    expect(unwrapped.name).toBe('dsh-plugin-vscode-sidebar')
  ...
  it('registers the side card preferences schema with the documented defaults', async () => {
    ...
    expect(SIDEBAR_PREFS_NS).toBe('dsh-plugin-vscode-sidebar')
    ...
    // A new background job auto-opens the Jobs page too, and its streaming
    // output auto-expands (the autoSaveOnClose guard defaults OFF).
    expect(resolved.autoOpenJobs).toBe(true)
    expect(resolved.autoOpenJobStream).toBe(true)
    expect(resolved.autoSaveOnClose).toBe(false)
    ...
    expect(overridden).toEqual({ openByDefault: false, defaultWidthPercent: 45, autoOpenSubagent: true, autoOpenJobs: true, autoOpenJobStream: true, autoSaveOnClose: false, agentTerminalTools: false, bottomPanelAutoTerminal: true, terminalFontFamily: '', terminalFontSize: 13, interceptOpenPath: true, htmlViewerNoSandbox: false, htmlViewerDefaultUnsafe: false, browserNoSandbox: false, browserInterceptLinks: true, tabsEnabled: {}, viewersEnabled: {}, pluginSettings: {} })
  })
})
