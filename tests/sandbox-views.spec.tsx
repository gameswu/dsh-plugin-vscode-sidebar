...
import { HTML_IFRAME_SANDBOX } from '../src/client/sandbox.ts'
...
function previewProps(store: ReturnType<typeof createSidebarStore>) {
  return {
    store,
...
describe('HTML preview iframe sandbox', () => {
  it('renders the preview iframe with the exact sandbox tokens and no same-origin / top-navigation', () => {
    ...
    const html = renderToString(createElement(HtmlPreview, previewProps(store)))
  ...
  it('markdown preview keeps rendering markdown, not an iframe', () => {
    ...
    const html = renderToString(createElement(MarkdownPreview, {
      text: '# hi',
      mdRef: ref,
      onMouseUp: () => {},
      onScroll: () => {},
