/**
 * Tests for the VscodeSidebar service registry: register/dispose lifecycle,
...
import { createVscodeSidebarService, SIDEBAR_FEATURES, SIDEBAR_SERVICE_VERSION } from '../src/client/service.ts'
import { createSidebarStore, allLeaves, makeDefaultState, openDiffTab, sanitizeState } from '../src/client/state.ts'

describe('VscodeSidebar service', () => {
...
describe('version and feature detection (v0.12.0)', () => {
  it('reports the plugin version in lockstep with package.json', () => {
    ...
    expect(createVscodeSidebarService(createSidebarStore()).version).toBe(SIDEBAR_SERVICE_VERSION)
  })

  it('advertises every v0.12.0 capability in the features list', () => {
    const service = createVscodeSidebarService(createSidebarStore())
