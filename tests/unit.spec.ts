...
import { compareEntries, isWithin, listDirectory, parentOf, rootLabel, requireAbsolute } from '../src/fs-tree.ts'
...
describe('fs-tree', () => {
  it('sorts directories first, then names case-insensitively', () => {
    const rows = [
      ...
      { name: '.dotfile', path: '/x/.dotfile', isDir: false, hidden: false },
    ]
    expect(rows.sort(compareEntries).map(row => row.name)).toEqual(['A', '.dotfile', 'a.txt', 'b.txt'])
  })

  it('lists dotfiles normally and always drops the .git metadata directory', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'dsh-fstree-'))
    try {
      mkdirSync(join(dir, '.git'))
      writeFileSync(join(dir, '.env'), 'A=1')
      writeFileSync(join(dir, 'a.txt'), 'x')
      const listing = await listDirectory(dir)
      expect(listing.entries.map(entry => entry.name)).toEqual(['.env', 'a.txt'])
      // No repo context at this layer: nothing is flagged hidden here.
  ...
  it('accepts absolute paths and rejects relative ones', () => {
    ...
    // (the drive letter follows the process cwd, so assert its shape).
    expect(requireAbsolute('/a/b')).toMatch(process.platform === 'win32' ? /^[A-Za-z]:[\\/]a[\\/]b$/ : /^\/a\/b$/)
    if (process.platform === 'win32') {
...
describe('git parsing', () => {
  ...
  it('parses log rows with unit separators (full hash + refs + parents)', () => {
    const rows = parseLogLines(
      ...
      + 'aaa0000\x1fRoot\x1fBob\x1f2024-01-03 10:00:00 +0800\x1faaa0000aaa0000aaa0000aaa0000aaa0000aaa0000\x1f\n',
    )
    expect(rows).toEqual([
      {
        hash: 'abc1234',
      ...
      },
      {
        ...
        hashFull: 'aaa0000aaa0000aaa0000aaa0000aaa0000aaa0000',
        refs: '',
        parents: [],
      },
    ])
...
describe('pty helpers', () => {
  it('falls back from an empty SHELL to a usable shell', () => {
    ...
    // The empty-SHELL fallback is the platform's default shell: powershell
...
describe('open-path interception wiring', () => {
  it('registerOpenPathInterception routes chat opens into the editor tab and restores on dispose', async () => {
    ...
    const ctx = {
      ...
      workspaces: funnel,
      vscodeSidebar: { openTab: (seed: unknown) => { opened.push(seed as Record<string, unknown>) } },
