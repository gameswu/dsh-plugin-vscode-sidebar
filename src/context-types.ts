...
import type { VscodeSidebarService } from './client/service.ts'
...
/**
 * The harness's own owner-scoped PTY registry face (mirror of
 * `@deepseek-ai/dsh-terminal`'s TerminalSessionService) — the terminals the
 * MAIN agent drives through the HARNESS's terminal surface (not this
 * plugin's `terminal_*` tools). Pull-based only: the service emits no
 * events, so the sidebar's streaming view polls `read`. Optional — the
 * tasks page simply omits harness terminals when the deployment lacks it.
 */
export interface SidebarTerminalsService {
  /** Snapshots of one live owner's published PTY sessions. */
  list(owner: SidebarAgent): Array<{
    sessionId: string
    name?: string
    type: string
    pid?: number
  ...
  /** Read one bounded scrollback page (newest-relative offsets). */
  read(owner: SidebarAgent, id: string, request?: {
    offset?: number
    count?: number
  }): {
    text: string
 ...
 * namespace registry under `vscodeSidebar`.
 */
...
/**
 * The client settings-scope binder face (mirror of `dsh-client-ui-settings`'
 * `SettingsScopeBinder`, contract from `dsh-client-runtime`). Features bind
 * their settings namespace through it and receive a live scope: every Host
 * settings commit (the official plugin-config page, the feature's own
 * settings section, external document edits) pushes a fresh snapshot to the
 * bound scope, so preference changes apply to the running page immediately.
 * Optional in the client — deployments without the settings surface have no
 * binder, and consumers fall back to their own one-shot read.
 */
export interface SidebarSettingsScopeBinder {
  bind<T>(spec: {
    namespace: string
...
declare module 'cordis' {
  interface Context {
     ...
     * dictionaries under the `vscodeSidebar` namespace. Client side only.
     */
    ...
    /**
     * The client settings-namespace scope binder (`ctx.get('settingsScope')`;
     * optional — the sidebar subscribes through it for LIVE preference
     * changes and falls back to its one-shot fenced settings read without
     * it). Client side only.
     */
    ...
    /**
     * The harness's own PTY registry (`ctx.get('terminals')`; optional — the
     * tasks page streams the MAIN agent's harness-created terminals through
     * it; absent in deployments without the terminal surface).
     */
     ...
     */
    vscodeSidebar: VscodeSidebarService
    /**
