...
export const zh = {
  ...
  rename: '重命名',
  deleteEntry: '删除',
  deleteEntryDesc: '确定删除「{name}」吗？此操作不可撤销。',
  entryName: '名称',
  save: '保存',
  saved: '已保存',
  saveShortcut: 'Ctrl+S 保存',
  unsaved: '未保存',
  unsavedCloseTitle: '未保存的更改',
  unsavedCloseDesc: '「{name}」有未保存的更改，是否在关闭前保存？',
  unsavedCloseDiscard: '不保存',
  saveFailed: '保存失败',
  ...
  settingsJobStreamTitle: '自动展开新任务的流式输出',
  settingsJobStreamDesc: '新的后台任务或代理终端出现时，在任务管理页自动展开其终端风格流式输出（无需点击卡片）；关闭后需点击卡片展开',
  settingsAutoSaveCloseTitle: '关闭含未保存改动的标签时自动保存',
  settingsAutoSaveCloseDesc: '开启后，关闭有未保存修改的编辑器标签会先自动保存再关闭，不再弹出确认；保存失败时仍会询问（默认关闭）',
  ...
  agentTerminals: 'Agent 终端',
  agentTerminalsCount: '{count} 个终端',
  agentTerminalsCountRunning: '{count} 个终端 · {running} 运行中',
  agentTerminalRunning: '运行中',
  agentTerminalExited: '已退出',
  ...
  jobKillError: '终止失败',
  gitSelectRepo: '选择仓库',
  gitReposFound: '检测到 {count} 个子仓库，请在上方选择要查看的仓库',
  gitShowDiff: '显示差异',
  gitHideDiff: '收起差异',
  gitAheadBehind: '领先 {ahead} · 落后 {behind}',
  settingsGitExcludeTitle: '仓库探测排除目录',
  settingsGitExcludeDesc: '在子目录中探测 git 仓库时跳过这些目录（逗号/空格分隔）；node_modules 等默认目录始终排除',
  settingsGitExcludePlaceholder: 'vendor, dist',
  settingsGitScanDepthTitle: '仓库扫描深度',
  settingsGitScanDepthDesc: '源代码管理页展示嵌套 git 仓库时向下扫描的最大层级（1–10，默认 3）；更深的仓库不会被列出',
  gitReposCount: '{count} 个仓库',
  settingsExplorerGitTitle: '按 Git 状态着色文件',
  settingsExplorerGitDesc: '源代码管理启用时，在文件浏览器中按 git 状态给文件着色并显示状态字母（M/A/D/R/?）',
}
...
export const en: Record<keyof typeof zh, string> = {
  ...
  rename: 'Rename',
  deleteEntry: 'Delete',
  deleteEntryDesc: 'Delete "{name}"? This cannot be undone.',
  entryName: 'Name',
  save: 'Save',
  saved: 'Saved',
  ...
  unsavedCloseDesc: '"{name}" has unsaved changes. Save them before closing?',
  unsavedCloseDiscard: "Don't save",
  ...
  settingsJobStreamTitle: 'Auto-expand the streaming output of new tasks',
  settingsJobStreamDesc: 'When a new background job or agent terminal appears, auto-expand its terminal-style streaming output on the Tasks page (no click needed); turn off to expand cards by clicking',
  settingsAutoSaveCloseTitle: 'Auto-save tabs with unsaved changes on close',
  settingsAutoSaveCloseDesc: 'When enabled, closing an editor tab with unsaved changes saves it first instead of asking; a failed save still falls back to the confirm dialog (off by default)',
  ...
  agentTerminals: 'Agent terminals',
  agentTerminalsCount: '{count} terminals',
  agentTerminalsCountRunning: '{count} terminals · {running} running',
  agentTerminalRunning: 'Running',
  agentTerminalExited: 'Exited',
  ...
  jobFollowOutput: 'Follow latest output',
  ...
  gitSelectRepo: 'Select repository',
  gitReposFound: 'Found {count} nested repositories — pick the one to inspect above',
  gitShowDiff: 'Show diff',
  gitHideDiff: 'Hide diff',
  gitAheadBehind: '{ahead} ahead · {behind} behind',
  settingsGitExcludeTitle: 'Repo discovery exclusions',
  settingsGitExcludeDesc: 'Skip these directories when scanning for nested git repositories (comma/space separated); node_modules & co are always excluded',
  settingsGitExcludePlaceholder: 'vendor, dist',
  settingsGitScanDepthTitle: 'Repository scan depth',
  settingsGitScanDepthDesc: 'How deep the source-control page scans for nested git repositories (1–10, default 3); deeper checkouts are not listed',
  gitReposCount: '{count} repositories',
  settingsExplorerGitTitle: 'Color files by git status',
  settingsExplorerGitDesc: 'With source control enabled, color explorer files by git status and show the status letter (M/A/D/R/?)',
}
 ...
 */
export const LOCALE_NS = 'vscodeSidebar'
