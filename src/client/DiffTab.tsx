  ...
  useEffect(() => {
    ...
    const load = async (): Promise<void> => {
      try {
        ...
        if (diff.kind === 'commit') {
          const result = await api.gitCommitDiff(scope, diff.hashFull, repo)
          if (!cancelled) setData({ diff: result.diff })
          return
        }
        let result = await api.gitDiff(scope, diff.path, diff.staged, repo)
        if (result.diff === '') {
          ...
          const other = await api.gitDiff(scope, diff.path, !diff.staged, repo)
        ...
        if (diff.untracked === true && !diff.staged) {
          const text = await api.fsRead(scope, diff.path, repo)
          if (!cancelled) {
  ...
  return (
    <div className={css.gitDiffTab}>
      <div className={css.gitDiffTabHeader}>
        {diff.kind === 'worktree' && <FileTypeIcon name={fileBaseName(diff.path)} size={14} />}
