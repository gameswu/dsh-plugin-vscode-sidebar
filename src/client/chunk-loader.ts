...
const defaultScriptLoader: ChunkScriptLoader = (src) => new Promise((resolve, reject) => {
  ...
  el.addEventListener('error', () => {
    el.remove()
    reject(new Error(`[dsh-plugin-vscode-sidebar] chunk script ${src} failed to load`))
  }, { once: true })
...
async function buildExternalsRequire(modules: ChunkModuleSystem): Promise<(spec: string) => unknown> {
  ...
  externalsRequire = (spec: string): unknown => {
    if (!table.has(spec)) {
      ...
      throw new Error(`[dsh-plugin-vscode-sidebar] chunk require('${spec}') missed the module table`)
    }
...
export function loadChunk(name: ChunkName): Promise<ChunkExports> {
  ...
  const task = (async (): Promise<ChunkExports> => {
    ...
    if (modules === undefined) {
      throw new Error(`[dsh-plugin-vscode-sidebar] chunk "${name}": client module system unavailable`)
    }
    await scriptLoader(CHUNK_URL(name))
    const factory = chunkRegistry()[name]
    if (typeof factory !== 'function') {
      throw new Error(`[dsh-plugin-vscode-sidebar] chunk "${name}" script did not register its factory`)
    }
