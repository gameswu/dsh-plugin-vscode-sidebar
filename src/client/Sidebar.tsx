...
import { Button, IconCloseFill14, Modal, Tooltip } from '@deepseek-ai/dsh-client-ui-primitives'
...
export function Sidebar(props: { ctx: Context; store: SidebarStore }) {
  ...
  useEffect(() => {
    ...
    if (ctx.vscodeSidebar?.isTabEnabled('subagent') === false) return
  ...
  useEffect(() => {
    ...
    if (ctx.vscodeSidebar?.isTabEnabled('terminal') === false) return
    ...
    ctx.vscodeSidebar?.openTab({ type: 'terminal' })
  ...
  /**
   * The canonical close: releases a closed terminal's pty immediately —
   * including when its socket is mid-reconnect, where the unmount close
   * frame never reaches the host and the process would hold the quota
   * until the grace ends. Agent terminals (tabId `agent:<uuid>`) close
   * through a different host route: the WS close frame is the primary
   * path (sent by TerminalView on unmount), and the agent-pty.close HTTP
   * route is the fallback when the WS is down.
   */
  const performClose = useCallback((paneId: string, tabId: string): void => {
    void paneId
    ...
    ctx.vscodeSidebar?.closeTab(tabId, sessionId === undefined ? undefined : { sessionId, cwd })
  ...
  }, [store, ctx.vscodeSidebar, sessionId, cwd])

  /** The pending unsaved-close confirmation (tab bar title + pane/tab ids). */
  const [closeConfirm, setCloseConfirm] = useState<{ paneId: string; tabId: string; title: string } | null>(null)

  // Global Ctrl/Cmd+S: saves the ACTIVE tab of the active pane when it is a
  // dirty editor (VSCode's save-anywhere semantics; monaco's own Ctrl+S
  // covers the focused-editor case).
  useEffect(() => {
  ...
  const actions: WorkbenchActions = useMemo(() => ({
    closeTab: (paneId, tabId) => {
      ...
      if (store.isTabDirty(tabId)) {
        ...
        setCloseConfirm({ paneId, tabId, title })
        return
      }
      performClose(paneId, tabId)
    },
    activateTab: (paneId, tabId) => {
      ...
      ctx.vscodeSidebar?.activateTab(tabId, sessionId === undefined ? undefined : { sessionId, cwd })
    },
  ...
  }), [store, sessionId, cwd, performClose])
  ...
  const tabIconOf = (tab: SidebarTab): ReactNode => {
    const filePath = tab.path ?? (tab.diff?.kind === 'worktree' ? tab.diff.path : undefined)
    if (filePath !== undefined && filePath !== '') {
      return <FileTypeIcon name={fileBaseName(filePath)} size={14} />
    }
    const descriptor = ctx.vscodeSidebar?.getTab(tab.type)
  ...
  const tabBadgeOf = (tab: SidebarTab): ReactNode => {
    ...
    try {
      ...
      console.error('[dsh-plugin-vscode-sidebar] tab badge error:', error)
      return null
    }
  ...
  return (
    <>
      ...
      */}
      ...
      {/* VSCode-style unsaved-close confirm: Save / Don't save / Cancel.
          Save closes only after the write actually succeeded. */}
      <Modal
        ...
        footer={(
          <>
            <Button variant="outline" onClick={() => {
              ...
              performClose(pending.paneId, pending.tabId)
            }}>
              {t('unsavedCloseDiscard')}
            </Button>
      ...
      >
        <p className={css.gitConfirmDesc}>{t('unsavedCloseDesc', { name: closeConfirm?.title ?? '' })}</p>
      </Modal>
    </>
  )
}
