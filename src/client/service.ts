/**
 * The VscodeSidebar client service: a registry that external plugins use
 * to contribute sidebar tab types and file previewers. The service is
 * published to the cordis context as `ctx.vscodeSidebar` (see
...
export interface FileViewerProps {
  ...
  customData?: unknown
  /**
   * The hosting editor tab's id: editors register their save callback
   * under it so the tab strip can show the dirty dot and the shell can
   * offer save-on-close / global Ctrl+S.
   */
  tabId?: string
}
...
/**
 * The registry service published as `ctx.vscodeSidebar`.
 */
export interface VscodeSidebarService {
 ...
 * - 'updateTab': VscodeSidebarService.updateTab
 * - 'openFile': VscodeSidebarService.openFile
 * - 'targetedOpen': VscodeSidebarService.openTab(seed, scope?)
...
function safeCall(fn: () => void): void {
  try {
    ...
    console.error('[dsh-plugin-vscode-sidebar] plugin callback error:', error)
  }
}

/**
 * Create one VscodeSidebar service bound to a store. The service owns the
 ...
 */
export function createVscodeSidebarService(store: SidebarStore): VscodeSidebarService {
  ...
  const registerTab = (descriptor: TabDescriptor): (() => void) => {
    if (tabs.has(descriptor.id)) {
      throw new Error(`[dsh-plugin-vscode-sidebar] tab type "${descriptor.id}" already registered`)
    }
  ...
  const registerFileViewer = (descriptor: FileViewerDescriptor): (() => void) => {
    if (viewers.has(descriptor.id)) {
      throw new Error(`[dsh-plugin-vscode-sidebar] file viewer "${descriptor.id}" already registered`)
    }
  ...
  const openTab = (seed: OpenTabSeed, scope?: SessionScope): void => {
    ...
    if (!isTabEnabled(seed.type)) {
      console.warn(`[dsh-plugin-vscode-sidebar] tab type "${seed.type}" is disabled in the side card settings`)
      return
    }
  ...
  if (key !== undefined) {
    ...
    for (const leaf of allLeaves(state.splits).concat(allLeaves(state.bottomSplits))) {
      ...
      if (existing !== undefined) {
        // Heal a stale title on the focused instance: an existing tab may
        // predate a locale switch or the pre-i18n hardcoded seed ('Explorer'
        // inside a zh UI) — the dedupe focus is the natural moment to
        // re-apply the descriptor's current localized title.
        const healed = existing.title !== tab.title
          ? patchTab(state, existing.id, { title: tab.title })
          : state
        return activateTabReducer(healed, leaf.id, existing.id)
      }
    }
  }
