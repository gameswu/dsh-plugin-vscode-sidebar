...
export type SidebarDiffRef =
  ...
  | { kind: 'commit'; hash: string; hashFull: string; subject: string; repo?: string }
...
export function makeDefaultState(width = PANEL_DEFAULT, panelOpen = true, seedExplorer = true): SidebarState {
  ...
  if (seedExplorer) {
    // The title follows the ACTIVE locale at seed time (the descriptor's
    // t('explorer')) — a hardcoded 'Explorer' here once produced an
    // English-titled tab inside a zh UI, which the title-keyed surface
    // discovery of other plugins (dsh-plugin-background) then saw as a
    // SECOND, distinct explorer surface.
    leaf.tabs = [{ id: uid('tab'), type: 'explorer', title: t('explorer') }]
 ...
 * tab descriptor through {@link VscodeSidebarService.openTab} / `dedupeKey`;
