...
export function EditorHost(props: {
  ...
  path: string
  title: string
  /** The hosting tab id (dirty-dot registry + save-on-close). */
  tabId?: string
}) {
  ...
  useEffect(() => {
      ...
      switch (action.kind) {
        ...
        case 'fetchFsRead':
          api.fsRead(scope, path).then((result) => {
            ...
            }, (head) => ctx.vscodeSidebar?.matchFileViewer(path, head), mediaUrlOf)
            apply(outcome)
          }).catch((error: unknown) => {
            if (cancelled) return
            setLoad({ status: 'error', message: error instanceof Error ? error.message : String(error) })
    ...
    apply(planFirstMatch(ctx.vscodeSidebar?.matchFileViewer(path), mediaUrlOf))
  ...
  return (
    <div className={css.editor}>
      <div className={css.editorHeader}>
        <FileTypeIcon name={fileBaseName(path)} size={14} />
      ...
      {load.status === 'ready' && createElement(load.viewer.component, {
        ctx, store, scope, path, title, tabId,
