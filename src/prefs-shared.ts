...
export const SIDEBAR_PREFS_NS = 'dsh-plugin-vscode-sidebar'
...
export interface SidebarPrefs {
  ...
  /**
   * Whether the Tasks page auto-EXPANDS the streaming output of a NEW
   * background job (or a new agent terminal) as soon as it appears, so the
   * terminal-style output is visible without clicking the row. On by
   * default; the switch lives under the Tasks card in the Side card
   * settings.
   */
  autoOpenJobStream: boolean
  /**
   * Whether closing an editor tab with unsaved changes saves it
   * automatically instead of asking. Off by default (the VSCode-style
   * Save / Don't save / Cancel confirm); when on, the close saves first —
   * a failed save falls back to the confirm dialog.
   */
