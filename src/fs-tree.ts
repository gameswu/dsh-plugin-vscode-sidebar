 ...
 * drops the `.git` metadata directory. Visibility follows git semantics, not
 * POSIX dotfiles: dot-prefixed entries show normally, and the wire layer
 * marks entries the enclosing repository's .gitignore ignores with
 * `hidden` so the explorer renders them DIMMED (VSCode-style) instead of
 * removing them (see the fs.tree route). Symlinks are reported as files
 * without probing their target — the explorer shows what dirent says,
 * keeping the read cheap for arbitrarily large levels.
 */
...
export async function listDirectory(path: string, maxEntries = 1000): Promise<SidebarFsListing> {
  ...
  try {
    for await (const dirent of level) {
      // Version-control metadata never belongs in the explorer tree.
      ...
      rows.push({
        ...
        // The gitignore dimming flag is stamped by the fs.tree route (this
        // level has no repo context of its own).
        hidden: false,
      })
    }
  } catch (error) {
