/**
 * tsdown build for dsh-plugin-vscode-sidebar: the host-half lib (lib/index.js and
 ...
 * it registers with the package-name id `dsh-plugin-vscode-sidebar` (the
 ...
 * (src/client/chunks/<name>.tsx). Each script assigns its factory to the
...
/**
 * One client bundle build: src/client/index.tsx compiled for the official
 * profile channel, registering with the package-name id
 ...
 *
 * Chunk css tags use the constant plugin id `dsh-plugin-vscode-sidebar` (matching
...
function chunkBundle(name: string): UserConfig {
  return {
    ...
    noExternal: (id: string) => (CLIENT_EXTERNALS.includes(id) ? undefined : true),
    plugins: [purityGatePlugin(), makeCssPlugin('dsh-plugin-vscode-sidebar')],
    outputOptions: {
...
/** MIME types for local css url() assets inlined as data URIs. */
...
/**
 * Rewrite local `url(...)` references in one stylesheet to data URIs (fonts
 * like monaco's codicon.ttf ship inside the package and must survive being
 * inlined as a <style> tag at runtime). Remote/data/fragment URLs pass
 * through untouched; unreadable files stay as-is (a 404, never a build
 * failure).
 */
async function inlineCssAssets(cssText: string, cssDir: string): Promise<string> {
  return cssText.replace(/url\(\s*['"]?([^'")]+)['"]?\s*\)/g, (whole, raw: string) => {
    const target = raw.trim()
    if (target === '' || /^(data:|https?:|\/\/|#|var\()/i.test(target)) return whole
    try {
      const filePath = resolvePath(cssDir, target.split(/[?#]/)[0]!)
      const ext = extname(filePath).toLowerCase()
      const mime = ASSET_MIME[ext]
      if (mime === undefined) return whole
...
function makeCssPlugin(pluginId: string): BuildPlugin {
  return {
    ...
    async load(virtualId: string) {
      ...
      // (xterm's stylesheet, monaco's editor css) is inlined verbatim.
      ...
      const cssText = await inlineCssAssets(source.toString('utf8'), dirname(fileId))
      return [
...
/** The monaco editor worker: a self-contained IIFE classic script the
 *  editor chunk loads via `new Worker('/sidebar/bundle/editor-worker.js')`
 *  (served by the bundle route's worker allowlist). */
const editorWorkerBundle: UserConfig = {
  entry: { 'editor-worker': 'src/client/workers/editor-worker.ts' },
  outDir: 'lib',
  format: ['iife'],
  platform: 'browser',
  dts: false,
  ...
  noExternal: () => true,
  plugins: [makeCssPlugin('dsh-plugin-vscode-sidebar')],
  outputOptions: {
    entryFileNames: 'client-editor-worker.js',
    codeSplitting: false,
  },
}
...
export default [
  ...
  clientBundle('dsh-plugin-vscode-sidebar', 'client.js'),
  // Lazy chunks, fetched on first use through the plugin's /sidebar/bundle
  // route (see src/client/chunk-loader.ts).
  ...CHUNKS.map(chunkBundle),
  // The editor worker script (loaded by the monaco chunk, not the loader).
  editorWorkerBundle,
